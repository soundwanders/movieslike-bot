...
const findSimilarMovies = async (queryMovie, releaseDate, genreMatches, actorMatches, languageMatches) => {
  try {
    ...
    // Initialize an empty array, will be populated with the movie data
    let filteredMovies = [];
    ...
    for (const movie of similarMovies) {
      ...
      if (genreMatches) {
        ...
        if (!genreMatchesLower || genreMatchesLower.some((genre) => genres.includes(genre))) {
      ...
      // If the movie matches all search criteria, add it to the results
      if (includeMovie) {
